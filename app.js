...
app.get("/impact/:impact", function(req, res){
    ...
    const url = `https://api.predicthq.com/v1/events/?limit=100&country=AE${impact_cond[req.params.impact]}&start.gte=${today}`;
    crossFetch.fetch(url, options)
    .then(response => { 
        if (!response.ok) {
            throw Error("Error");
        }
...
app.get("/category/:category/impact/:impact", function(req, res){
    ...
    // console.log(req.params);
    // console.log(impact_cond[req.params.impact]);
    const url = `https://api.predicthq.com/v1/events/?limit=100&country=AE&category=${req.params.category}${impact_cond[req.params.impact]}&start.gte=${today}`;
    crossFetch.fetch(url, options)
    .then(response => { 
        if (!response.ok) {
            throw Error("Error");
